...
let newArray = doubleTheArray([1, 2, 3, 8]);
...
let todos = [
  ...
  { createdAT: "2000-09-30", title: "mongodb", status: "pending" },
  { createdAT: "2000-09-30", title: "expressjs", status: "pending" },
  { createdAT: "2000-09-30", title: "nodejs", status: "pending" },
];
...
const todosDivision = () => {
  ...
  for (i = 0; i < todos.length; i++) {
      ...
      let {title,status} = todos[i];
      
    if (status == "completed") {
      completedTodos.push(title);
    } else if (status == "pending") {
      pendingTodos.push(title);
    }
  }
...
for(i=0;i<nepaliNames.length;i++){
  dummyUsers.push({name: nepaliNames[i],email:`${nepaliNames[i].toLowerCase()}@gmail.com`,password:`${nepaliNames[i].toLowerCase()}${i}`})
}
...
const findCountryCode = countries.find((el)=>el.name === "Nepal")
...
/*const findCountryCode = (country) => {
  ...
  let element2 = countries[i];
  if (element2.name == country || element2.path == country || element2.name.toUpperCase()==country.toUpperCase()){
    return element2.code;
  }
}
